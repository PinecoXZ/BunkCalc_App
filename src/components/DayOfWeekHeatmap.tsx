...
export const DayOfWeekHeatmap: React.FC<Props> = ({ records }) => {
  ...
  return (
    <section className="mb-8">
      ...
      <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 space-y-5 shadow-sm">
        {/* Heatmap Grid */}
        <div className="grid grid-cols-6 gap-2">
          {weekdayStats.map((item) => {
            ...
            return (
              <div
                key={item.dayIndex}
                className="bg-white dark:bg-slate-950 p-3 rounded-xl border border-slate-200/80 dark:border-slate-800 flex flex-col justify-between items-center text-center shadow-xs"
              >
                <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-slate-500">
                  {item.shortName}
                </span>

                <div className="w-full bg-slate-100 dark:bg-slate-800 h-16 rounded-lg my-2 flex flex-col justify-end overflow-hidden p-1">
                  <div
        ...
        {/* Highlight Insights */}
        {(bestDay || worstDay) && (
          <div className="grid grid-cols-2 gap-3 pt-2 border-t border-slate-200/80 dark:border-slate-800/80">
            {bestDay && (
